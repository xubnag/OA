...
public class UserController {
    ...
    @GetMapping("/login")
    @Operation(summary = "用户登录系统xubang")
    public R login(@Valid @RequestBody LoginForm form){
        HashMap param = JSONUtil.parse(form).toBean(HashMap.class);
        Integer userId =userService.login(param);
        R r = R.ok().put("userId",Objects.isNull(userId)?false:true);
        if(Objects.nonNull(userId)){
            StpUtil.setLoginId(userId);
            r.put("permissions",userService.searchUserPermissions(userId)).put("token",StpUtil.getTokenValue().toString());
        }
        return r;
    }
